...
Food type & Nutrient \#1 & Nutrient \#2 & Cost \\
\hline
1 & 2 & 0 & 20 \\
2 & 0 & 1 & 10 \\
3 & 3 & 2 & 31 \\
...
\end{tabular}
\end{center}
Nutrient requirements: 21 units of Nutrient \#1 and 12 units of Nutrient \#2
\end{frame}
...
\end{tabular}
\end{center}
Nutrient requirements: \\ \#1: 21, \#2: 12
\end{column}
\begin{column}{7cm}
\begin{itemize}
...
\color{red}gurobi> m = Model(name = 'diet')
gurobi> x1 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 20, vtype = GRB.CONTINUOUS, name = 'food.1')
gurobi> x2 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 10, vtype = GRB.CONTINUOUS, name = 'food.2')
gurobi> x3 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 31, vtype = GRB.CONTINUOUS, name = 'food.3')
gurobi> x4 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 11, vtype = GRB.CONTINUOUS, name = 'food.4')
gurobi> x5 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 12, vtype = GRB.CONTINUOUS, name = 'food.5')
gurobi> m.update()
gurobi> nut1_con = m.addConstr(2*x1 + 3*x3 + 1*x4 + 2*x5 >= 21, 'nutrient.1')
gurobi> nut2_con = m.addConstr(1*x2 + 2*x3 + 2*x4 + 1*x5 >= 12, 'nutrient.2')
gurobi> m.optimize()
Optimize a model with 2 rows, 5 columns and 8 nonzeros
Iteration    Objective       Primal Inf.    Dual Inf.      Time
       0    0.0000000e+00   1.125000e+01   0.000000e+00      0s
...
{\tiny
\begin{Verbatim} [commandchars=\\\{\}]
gurobi> m = Model(name = 'diet')
\color{red}gurobi> x1 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 20, vtype = GRB.CONTINUOUS, name = 'food.1')
\color{red}gurobi> x2 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 10, vtype = GRB.CONTINUOUS, name = 'food.2')
\color{red}gurobi> x3 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 31, vtype = GRB.CONTINUOUS, name = 'food.3')
\color{red}gurobi> x4 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 11, vtype = GRB.CONTINUOUS, name = 'food.4')
\color{red}gurobi> x5 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 12, vtype = GRB.CONTINUOUS, name = 'food.5')
gurobi> m.update()
gurobi> nut1_con = m.addConstr(2*x1 + 3*x3 + 1*x4 + 2*x5 >= 21, 'nutrient.1')
gurobi> nut2_con = m.addConstr(1*x2 + 2*x3 + 2*x4 + 1*x5 >= 12, 'nutrient.2')
gurobi> m.optimize()
...
gurobi> x5 = m.addVar(lb = 0, ub = GRB.INFINITY, obj = 12, vtype = GRB.CONTINUOUS, name = 'food.5')
gurobi> m.update()
\color{red}gurobi> nut1_con = m.addConstr(2*x1 + 3*x3 + 1*x4 + 2*x5 >= 21, 'nutrient.1')
\color{red}gurobi> nut2_con = m.addConstr(1*x2 + 2*x3 + 2*x4 + 1*x5 >= 12, 'nutrient.2')
gurobi> m.optimize()
Optimize a model with 2 rows, 5 columns and 8 nonzeros
Iteration    Objective       Primal Inf.    Dual Inf.      Time
       0    0.0000000e+00   1.125000e+01   0.000000e+00      0s
...
gurobi> nut1_con = m.addConstr(2*x1 + 3*x3 + 1*x4 + 2*x5 >= 21, 'nutrient.1')
gurobi> nut2_con = m.addConstr(1*x2 + 2*x3 + 2*x4 + 1*x5 >= 12, 'nutrient.2')
\color{red}gurobi> m.optimize()
\color{red}Optimize a model with 2 rows, 5 columns and 8 nonzeros
\color{red}Iteration    Objective       Primal Inf.    Dual Inf.      Time
\color{red}       0    0.0000000e+00   1.125000e+01   0.000000e+00      0s
\color{red}       2    1.3100000e+02   0.000000e+00   0.000000e+00      0s

\color{red}Solved in 2 iterations and 0.00 seconds
\color{red}Optimal objective  1.310000000e+02
\end{Verbatim}
}
\end{frame}
...
\item Constr object attributes
    \begin{itemize}
    \item nut1\_con.Pi - shadow price, change in objective/change in RHS
    \item nut1\_con.Slack - difference between LHS and RHS
    \end{itemize}
\end{itemize}
\end{frame}
...
m.Update();
GRBConstr con1 = m.AddConstr(2 * x1 + 3 * x3 + 1 * x4 + 2 * x5 >= 21, "nutrient.1");
GRBConstr con2 = m.AddConstr(1 * x2 + 2 * x3 + 2 * x4 + 1 * x5 >= 12, "nutrient.2");
m.Optimize();
...
\item How would the optimal cost change if we required an extra 0.1 units of nutrient \#1?
\end{itemize}
\end{frame}
...
\end{eqnarray}
Suppose we have pills that contain a single unit of nutrient \#1 or nutrient \#2. How much would someone solving the diet problem be willing to pay for these pills?
\end{frame}

\begin{frame}
\frametitle{Duality}
\begin{itemize}
\item Let $\pi_1, \pi_2$ be the price to be charged for a nutrient \#1, \#2 pill.
\item We wish to maximize total revenue of $v = 21 \pi_1 + 12 \pi_2$.
\item We must charge prices that are competitive with the prices of the five food types.
    \begin{itemize}
    \item Each ounce of food type 1 provides 2 units of nutrient \#1 at a cost of 20, so $2\pi_1 \le 20$.
    \item For food type 2, $\pi_2 \le 10$.
    \item For food type 3, $3\pi_1 + 2\pi_2 \le 31$.
    \item For food type 4, $\pi_1 + 2\pi_2 \le 11$.
    \item For food type 5, $2\pi_1 + \pi_2 \le 12$.
    \end{itemize}
\item We don't want to give away our pills, so $\pi_1, \pi_2 \ge 0$.
\end{itemize}
\end{frame}
...
The ``dual'' problem:
\begin{eqnarray}
v^* = \max_\pi && 21 \pi_1 + 12 \pi_2 \nonumber \\
\mbox{s.t.} && 2\pi_1 + 0\pi_2 \le 20 \nonumber \\
&& 0\pi_1 + 1\pi_2 \le 10 \nonumber \\
&& 3\pi_1 + 2\pi_2 \le 31 \nonumber \\
&& 1\pi_1 + 2\pi_2 \le 11 \nonumber \\
&& 2\pi_1 + 1\pi_2 \le 12 \nonumber \\
...
\item How are $\pi_1^*$ and $\pi_2^*$ related to solution of the original diet problem?
\item Multiply constraint \#1 in the original diet problem by $\pi_1^*$, constraint \#2 by $\pi_2^*$, and add the results.
    \begin{itemize}
...
Gurobi computes $\pi$ for us even when we solve the primal. Recall that in the optimal solution to the diet problem example, only $x_4$ and $x_5$ were non-zero. Letting $b_1$ and $b_2$ be nutrient requirements, we have
\begin{eqnarray}
x_4 + 2 x_5 &=& b_1 \nonumber \\
2 x_4 + x_5 &=& b_2 \nonumber
\end{eqnarray}
...
\begin{eqnarray}
z &=& 11x_4 + 12 x_5 \nonumber \\
&=& 11(-1/3 b_1 + 2/3 b_2) + 12(2/3 b_1 - 1/3 b_2) \nonumber \\
&=& 13/3 b_1 + 10/3 b_2 \nonumber
\end{eqnarray}
So, $\pi_1 = 13/3$ and $\pi_2 = 10/3$.
\end{frame}
...
\item Consider food 3, which costs 31 per ounce and provides 3 units of nutrient \#1 and 2 of nutrient \#2.
\item Nutrient \#1 is priced at 13/3 per unit, and nutrient \#2 at 10/3 per unit.
...
\item Suppose we introduce a new food that costs 20 per ounce and provides 2 units of nutrient \#1 and 3 units of nutrient \#2. Should we include this new food in our diet? Do we need to reoptimize?
\end{itemize}
\end{frame}
