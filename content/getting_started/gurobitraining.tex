...
\begin{itemize}
\item Objects and Methods you will need (Python equivalent)
    \begin{itemize}
    \item Model (GRBModel)
        \begin{itemize}
...
\begin{frame} [containsverbatim]
\frametitle{Diet Problem Implemented in Python(CONT.)}
\tiny
\begin{verbatim}
m.write('diet.lp')
!type diet.lp
...
\begin{itemize}
\item Model(name, env)
\item Model.AddVar(double lb, double ub, double obj, char type, String name)
\item Model.AddConstr(GRBLinExpr lhs, char sense, GRBLinExpr rhs, String name)
\item Get and Set methods access attributes of Model, Var, and Constr objects
...
#for loop alternative to quicksum method,
# using overloading (+= operator, not advisable in .NET or available in Java)
total_weight = LinExpr()
for i in I:
    total_weight += weights[i]*item_selected[i]
print total_weight

#API's of Java and .NET provided methods
total_weight = LinExpr()
for i in I:
...
#Method that takes in data, returns Model	
def get_knapsack_model(capacity, weights, values):
    items = range(len(weights))
    m = Model()
    m.ModelSense = GRB.MAXIMIZE
    item_selected = [m.addVar(ub=1, obj=values[item], name="item_selected." + str(item))for item in items]
    m.update()
